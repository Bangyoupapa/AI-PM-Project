        ...
        {(["upload", "preview", "done"] as Step[]).map((s, i) => (
          <div key={s} className="flex items-center gap-2">
            <div className={cn(
              ...
              (i < ["upload", "preview", "done"].indexOf(step)) ? "bg-green-500 text-white" : "bg-muted text-muted-foreground"
            )}>
              {i + 1}
            </div>
            <span className={cn("hidden sm:inline", step === s ? "text-foreground font-medium" : "text-muted-foreground")}>
              {s === "upload" ? "上傳檔案" : s === "preview" ? "確認資料" : "完成"}
            </span>
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-3">
            <Button onClick={handleConfirm} disabled={loading || preview.valid.length === 0}>
              {loading ? "匯入中…" : `確認匯入 ${preview.valid.length} 筆`}
            </Button>
        ...
        <div className="flex flex-col items-center gap-6 py-12">
          ...
          <div className="text-center">
            ...
            <p className="text-muted-foreground mt-1">
              成功匯入 <span className="font-bold text-green-700">{result.successRows}</span> 筆
              {result.failedRows > 0 && <>，失敗 <span className="font-bold text-red-700">{result.failedRows}</span> 筆</>}
            </p>
          </div>
          <Button onClick={() => router.push("/parts")}>前往料件清單</Button>
        </div>
      )}
    </div>
  );
